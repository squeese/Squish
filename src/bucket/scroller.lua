...
do
  ...
  function createCell(scroller, row, rowIndex, colIndex, prev)
    local cell = CreateFrame('frame', nil, row, "BackdropTemplate")
    cell:SetBackdrop(MEDIA:BACKDROP(true, true, 1, 1))
  ...
  function createRow(scroller, rowIndex, prev)
    local row = CreateFrame('frame', nil, scroller.frame, "BackdropTemplate")
    row:SetPoint('LEFT', 0, 0)
    row:SetPoint('RIGHT', 0, 0)
    row:SetBackdrop(MEDIA:BACKDROP(true, true, 1, 1))
