...
local function OnAttributeChanged(self, key, value)
  if key == 'unit' and self.unit ~= value then
    if value == nil and self.unit ~= nil then
      self:UnregisterAllEvents()
      Range:Unregister(self)
    elseif value ~= nil and self.unit == nil then
...
function Q.Party(parent, ...)
  ...
  function header:initialConfigFunction(name)
    ...
    -- button.leader:SetPoint("BOTTOMLEFT", 4, 4)
    ...
    -- button.assistant:SetPoint("BOTTOMLEFT", 20, 4)
    ...
    --button.role:SetPoint("BOTTOMLEFT", 36, 4)
  ...
  RegisterAttributeDriver(header, 'state-visibility', '[group:raid] hide; show')
